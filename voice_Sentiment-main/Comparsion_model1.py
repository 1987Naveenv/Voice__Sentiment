...
# Comparison Model 1 - LSTM + DenseNet121
class LSTM_DenseNet_AVClassifier(nn.Module):
    def __init__(self, num_classes):
        super().__init__()

        # Audio branch: 2-layer LSTM
        ...
        self.audio_fc = nn.Linear(128 * 2, 128)  # Bi-directional

        # Video branch: DenseNet121
        ...
        self.video_fc = nn.Linear(1024, 128)
    ...
    def forward(self, audio, video):
        # Audio: (B, 1, T) → (B, T, 1)
        audio = audio.squeeze(1).permute(0, 2, 1)
        lstm_out, _ = self.audio_lstm(audio)
        lstm_feat = lstm_out[:, -1, :]  # Last time step
        audio_feat = self.audio_fc(lstm_feat)

        # Video
        video_feat = self.video_fc(self.video_branch(video))
...
# Create DataFrame from folder structure
